...
function ServiceCard({ title, items }: { title: string; items: ReactNode[] }) {
  return (
    ...
    >
      <h3 className="mb-4 border-b border-gray-200 pb-3 text-[2rem] font-bold text-navy">{title}</h3>
      <ul className="space-y-4 text-[26px] text-foreground/95">
        {items.map((item, index) => (
          item === '__SUBHEAD_AS_WELL_AS__' ? (
            <li key={index} className="pl-5 pt-1 text-[26px] font-bold text-foreground/95">
              As well as:
            </li>
          ) : (
